/*
* Wild-Mustango: Modified version to accept destination framebuffer via argument
*/
...
int process(char * destFb) {
    ...
    fbfd = open((const char *)destFb, O_RDWR);
    if (fbfd == -1) {
...
int main(int argc, char *argv[]) {
	...
	switch (argc) {
		case 2: {
				...
				return process(destFb);	
    			break;
			}
		
		default:
			{
				printf("This program will duplicate content from fb0 to destination fb.\n");
				printf("Destination framebuffer expected\n");
				printf("Usage example: fbcp /dev/fb2 \n");
